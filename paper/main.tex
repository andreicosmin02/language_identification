...
\author{
\IEEEauthorblockN{Andrei-Cosmin Achim}
\IEEEauthorblockA{
Transilvania University of Brasov\\
Email: cosmin.achim@student.unitbv.ro
}
}
\date{}
...
\begin{itemize}
    ...
    \item noise conditions: clean, no-diacritics, and synthetic-typo.
\end{itemize}
...
The dataset used in the final experiments is the Hugging Face dataset \emph{agentlans/high-quality-multilingual-sentences}.\footnote{\url{https://huggingface.co/datasets/agentlans/}} From the available language-specific configurations, only EN, RO, FR, DE, and ES are retained. For each language, 2000 examples are sampled after cleaning and filtering.
...
\begin{itemize}
    ...
    \item \textbf{no-diacritics}: diacritics removed with a deterministic normalization function;
    \item \textbf{synthetic-typo}: one deterministic synthetic character substitution per text.
\end{itemize}
...
Each model is trained separately for each length bucket on the \texttt{train} split with \texttt{clean} texts only. The \texttt{val} split is used only for intermediate inspection and sanity checking, not for hyperparameter tuning in the current version. The main reported results in this paper are the \texttt{test} results for the same length bucket under all three conditions.
...
\centering
\includegraphics[width=\linewidth]{../results/plots/accuracy_by_length_clean.png}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{../results/plots/accuracy_by_noise.png}
\end{minipage}
\caption{Compact visual summary of the benchmark. Left: average accuracy on clean text as a function of text length. Right: average accuracy by noise condition across models.}
...
This confirms the central hypothesis of the project: short-text LID becomes significantly harder when the available context drops to only a few words. With 10 words, all three models are highly competitive. With 3 words, the gap between robust models and the lexical baseline becomes much wider, which is also visible in Fig.~\ref{fig:benchmark_summary}.
...
In this setup, French is the most difficult language and German the easiest. This is an empirical observation rather than a universal claim. One possible explanation is that French is affected both by the loss of diacritics and by short lexical overlap with the other Romance languages in the benchmark, especially Spanish and Romanian. By contrast, English does not rely strongly on diacritics, so the no-diacritics condition removes less language-specific information. German, on the other hand, may benefit from distinctive orthographic patterns captured well by character n-grams and subword models.
...
Third, fastText is particularly attractive when a strong performance-speed trade-off is needed. It is not the absolute best model on average, but it performs best or near-best on the 3-word clean and noisy conditions in this experiment while remaining much faster than the SVM. The speed results also confirm that TF-IDF with Logistic Regression is the fastest baseline overall, while char\_svm is the slowest of the three tested models.
